...
ARP_HEADER_FORMAT = '!2s2sBBH'  # Hardware type, Protocol type, Hardware size, Protocol size, Operation
ARP_IPV4 = b'\x08\x00'
ARP_ETHERNET = b'\x00\x01'
IPV4_SIZE = 4
ETHERNET_SIZE = 6
...
class ARP(LayerParser):
    ...
    def encapsulate(self, sender_hardware, sender_protocol, target_hardware, target_protocol, operation):
        hardware_type = ARP_ETHERNET
        protocol_type = ARP_IPV4

        hardware_size = ETHERNET_SIZE
        ...
        sender_protocol_bytes = bytes.fromhex(sender_protocol.replace('.', ''))
        target_hardware_bytes = bytes.fromhex(target_hardware.replace(':', ''))
        target_protocol_bytes = bytes.fromhex(target_protocol.replace('.', ''))
